...
from whitening import compute_matrices
...
def homoscedastic_noise(sigma, n_components):
    I = np.eye(n_components)
    noise = sigma ** 2 * I
    return I, noise, sigma
...
def _ica_def(X, tol, g, fun_args, max_iter, w_init):
    ...
    cov, V, P, U, Lambda, G, Theta = compute_matrices(X)
    del cov, V, P, U, G, Theta

    I, noise, sigma = homoscedastic_noise(sigma=0, n_components=n_components)
    coeff_init = (I + sigma ** 2 * np.linalg.inv(Lambda - noise))

    # j is the index of the extracted component
    for j in range(n_components):
        w = w_init[j, :].copy()
        w /= np.sqrt((w ** 2).sum())
        coeff = coeff_init[j, j].copy()

        for i in range(max_iter):
            gwtx, g_wtx = g(np.dot(w.T, X), fun_args)

            w1 = (X * gwtx).mean(axis=1) - g_wtx.mean() * w * coeff
...
class FastICA(BaseEstimator, TransformerMixin):
    ...
    *A. Hyvarinen and E. Oja, Independent Component Analysis:
    ...
    """
    def __init__(self, n_components=None, algorithm='deflation', whiten=True,
    ...
    def sum_square_residual(self, X, recovered):
        diff = X - recovered
        sq = np.power(diff, 2)
        ssr = np.sqrt(np.sum(sq))
        return ssr

    def score1(self, X):
        """
        Compute the MSE for model selection
        ...
        recovered = self.inverse_transform(sources)
        return self.sum_square_residual(X, recovered)

    def score(self, X):
        sources = self.fit_transform(X)
        gs, gs_ = _logcosh(sources, fun_args={})
        gs = gs.mean(axis=0)
        v = np.ones(shape=sources.shape[1])
        gv = 0.3745672074914517 * v
        score = np.sum(np.power(gs - gv, 2)) / self.n_components
        return score
